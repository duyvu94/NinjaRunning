...
public class HelpScreen implements Screen, InputProcessor {
	...
	public HelpScreen(Game myGame) {
		...
		backButton = new SimpleButton(gameWidth/2-100, 400, 200, 45, AssetLoader.backButtonUp, AssetLoader.backButtonDown);
	}
	...
	public void render(float delta) {
		...
		batcher.draw(AssetLoader.leftTouch, gameWidth/12, gameHeight/3, gameWidth/3, gameHeight/4);
		batcher.draw(AssetLoader.rightTouch, gameWidth/14 + gameWidth/2, gameHeight/3
				, gameWidth/3, gameHeight/4);

		
		font.getData().setScale(0.8f, -0.8f);
		font.draw(batcher, "Touch the left side to throw Kunai", gameWidth/14, gameHeight/2+gameHeight/6);
		font.draw(batcher, "Touch the right side to jump", gameWidth/2 + gameWidth/20, gameHeight/2+gameHeight/6);
